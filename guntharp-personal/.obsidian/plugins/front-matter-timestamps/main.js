...
var FrontMatterTimestampsPlugin = class extends import_obsidian.Plugin {
  ...
  async handleFileChange() {
    ...
    if (this.isPathExcluded(currentFile.path))
      return;
    const newFileTolerance = 0.3;
    const ctime = (0, import_obsidian.moment)(currentFile.stat.ctime);
    const mtime = (0, import_obsidian.moment)(currentFile.stat.mtime);
  ...
  async handleFileCreate(file) {
    ...
    const currentTime = (0, import_obsidian.moment)().format("YYYY-MM-DDTHH:mm:ssZ");
    try {
      await new Promise(
  ...
  async updateModifiedTime(file) {
    ...
    if (!file || !file.path || this.isPathExcluded(file.path))
      return;
    try {
      if (file.extension !== "md") {
        if (this.settings.debug) {
